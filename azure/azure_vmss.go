...
func (ss *scaleSet) getVmssVM(nodeName string, readType int) (string, string, *compute.VirtualMachineScaleSetVM, error) {
	getter := func(nodeName string) (string, string, *compute.VirtualMachineScaleSetVM, error) {
		cached, err := ss.vmssVMCache.Get(vmssVirtualMachinesKey, readType)
		if err != nil {
			return "", "", nil, err
		}
...
func (ss *scaleSet) getVmssVMByInstanceID(resourceGroup, scaleSetName, instanceID string, readType int) (*compute.VirtualMachineScaleSetVM, error) {
...
func (ss *scaleSet) EnsureHostInPool(service *v1.Service, nodeName types.NodeName, backendPoolID string, vmSetName string, isInternal bool) error {
	...
	if err = ss.deleteCacheForNode(vmName, allowUnsafeRead); err != nil {
		return err
	}
...
func (ss *scaleSet) ensureBackendPoolDeletedFromNode(service *v1.Service, nodeName, backendPoolID string) error {
	...
	// Invalidate the cache since we would update it.
	if err = ss.deleteCacheForNode(nodeName, allowUnsafeRead); err != nil {
		return err
	}
