...
func (az *Cloud) getVirtualMachine(nodeName types.NodeName, readType int) (vm compute.VirtualMachine, err error) {
	vmName := string(nodeName)
	cachedVM, err := az.vmCache.Get(vmName, readType)
	if err != nil {
		return vm, err
	}
...
func (az *Cloud) getRouteTable(readType int) (routeTable network.RouteTable, exists bool, err error) {
	cachedRt, err := az.rtCache.Get(az.RouteTableName, readType)
	if err != nil {
...
func (az *Cloud) getAzureLoadBalancer(name string, readType int) (lb network.LoadBalancer, exists bool, err error) {
	cachedLB, err := az.lbCache.Get(name, readType)
	if err != nil {
		return lb, false, err
	}
...
func (az *Cloud) getSecurityGroup(readType int) (nsg network.SecurityGroup, err error) {
	if az.SecurityGroupName == "" {
		return nsg, fmt.Errorf("securityGroupName is not configured")
	}

	securityGroup, err := az.nsgCache.Get(az.SecurityGroupName, readType)
	if err != nil {
		return nsg, err
	}
